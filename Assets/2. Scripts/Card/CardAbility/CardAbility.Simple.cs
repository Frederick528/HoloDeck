...
public partial class CardAbility
{
    ...
    private void SettingCondition(Card card, List<Func<UniTask<bool>>> conditionTasks, ref Action immediate, ref Action failure, ref Action success)
    {
        ...
        if ((card.Data.Discard != 0 || (card.Data.Discard == 0 && card.Data.DiscardOrder >= 0)) || (card.Data.Remove != 0 || (card.Data.Remove == 0 && card.Data.RemoveOrder >= 0)))
        {
            ...
            success += () => CardManager.Instance.SetCardState(2);
    ...
    private void BuildBaseAbilities(Card card, List<(float order, AbilityTag tag, Func<PlayContext, UniTask> task)> tasks)
    {
        ...
        if (card.Data.DamageOrder >= 0)
        {
            if (card.Data.CardTag == CardTag.SingleAttack)
                ...
                tasks.Add((card.Data.DamageOrder, AbilityTag.Attack, (ctx) => AllAttackAB(card, GetCrit(), ctx)));
            else if (card.Data.CardTag == CardTag.RandomAttack)
                tasks.Add((card.Data.DamageOrder, AbilityTag.Attack, (ctx) => RandomAttackAB(card, GetCrit(), ctx)));
        }

        // 4단계: 방어 (공격 후 실드 형성)
        if (card.Data.ShieldOrder >= 0)
        {
            tasks.Add((card.Data.ShieldOrder, AbilityTag.Shield, (ctx) => ShieldAB(card, ctx)));
        }

        // 5단계: 드로우 (유틸리티)
        if (card.Data.DrawOrder >= 0)
        {
            tasks.Add((card.Data.DrawOrder, AbilityTag.Draw, (ctx) => DrawAB(card, ctx)));
        }

        if (card.Data.HPOrder >= 0)
        {
            tasks.Add((card.Data.HPOrder, AbilityTag.ChangeHP, (ctx) => HpEffectAB(card, ctx)));
        }

        if (card.Data.DiscardOrder >= 0)
        {
            tasks.Add((card.Data.DiscardOrder, AbilityTag.PostEffect, (ctx) => ConfirmedDiscardAB(card, ctx)));
        }

        if (card.Data.RemoveOrder >= 0)
        {
            tasks.Add((card.Data.RemoveOrder, AbilityTag.PostEffect, (ctx) => ConfirmedRemoveAB(card, ctx)));
        }
    }
